...
class ValueEncoder(nn.Module):
    def __init__(self, value_dim, hidden_dim, single_object=False):
        ...
        self.distributor = MainToGroupDistributor()
        self.fuser = FeatureFusionBlock(256, 256, value_dim, value_dim)
        if hidden_dim > 0:
...
class ValueEncoderViT(nn.Module):
    def __init__(self, network, value_dim, hidden_dim, single_object=False):
        super().__init__()
    ...
    def forward(self, image, image_feat_f16, h, masks, others, is_deep_update=True):
        ...
        assert g.shape[-2:] == (1024, 1024), f'Invalid Shape {g.shape}'
        g = self.network(g)
    ...
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Pad
        h, w = x.shape[-2:]
        padh = self.network.img_size - h
        padw = self.network.img_size - w
