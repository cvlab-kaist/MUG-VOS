...
from dataset.vid_sam_dataset import VID_SAM_Dataset
# from dataset.vos_sam_datset import VID_SAM_Dataset
from dataset.BURST_dataset import BURST_Dataset

from dataset.filtered_affine_visa_dataset import ViSA_Dataset as Filtered_ViSA_Dataset
from dataset.affine_visa_dataset import ViSA_Dataset
...
for si, stage in enumerate(stages_to_perform):
    ...
    if local_rank == 0:
        ...
        model = SamTrainer(config, logger=logger, 
        ...
        model = SamTrainer(config, local_rank=local_rank, world_size=world_size).train()
    ...
    def renew_vos_loader(max_skip, finetune=False, transform=None):
        ...
        yv_dataset = VID_SAM_Dataset(path.join(yv_root, 'JPEGImages'), 
                            path.join(yv_root, 'Annotations'), max_skip//5, is_bl=False, subset=load_sub_yv(), num_frames=config['num_frames'], finetune=finetune, transform=transform)
        # davis_dataset = VID_SAM_Dataset(path.join(davis_root, 'JPEGImages', '480p'), 
        #                     path.join(davis_root, 'Annotations', '480p'), max_skip, is_bl=False, subset=load_sub_davis(), num_frames=config['num_frames'], finetune=finetune, transform=transform)
    ...
    def renew_burst_loader(max_skip, finetune=False, transform=None):
        burst_path = '/media/dataset3/video_seg/dataset/BURST'
        burst_anno_path = '/media/dataset3/sj/datasets/BURST/annotations'
        burst_dataset = BURST_Dataset(burst_path, burst_anno_path, max_skip, is_bl=False, num_frames=config['num_frames'], finetune=finetune, transform=transform )
        return burst_dataset

    def renew_bl_loader(max_skip, finetune=False):
        train_dataset = VID_SAM_Dataset(path.join(bl_root, 'JPEGImages'), 
    ...
    def renew_visa_loader(max_skip, finetune=False, transform=None, max_num_obj=3):
        import os
        visa_path = "/media/data3/video_seg/dataset/video_sam_v2"
        visa_anno_path = "/media/data3/video_seg/dataset/video_sam_v2-annotations"
        # train_dataset = ViSA_Dataset(visa_path, visa_anno_path, max_skip, is_bl=False, num_frames=config['num_frames'], finetune=finetune, transform=transform,
        #                              max_num_obj=max_num_obj)
        train_dataset = Filtered_ViSA_Dataset(visa_path, visa_anno_path, max_skip, is_bl=False, num_frames=config['num_frames'], finetune=finetune, transform=transform,
                                     max_num_obj=max_num_obj)
    ...
    if stage == '0':
        ...
        train_dataset = StaticTransformDataset(
            ...
            ], num_frames=config['num_frames'], transform=transform)
        ...
        train_sampler, train_loader = renew_visa_loader(5, transform=transform, max_num_obj=config['max_num_obj'])
        renew_loader = renew_visa_loader
    else:
        # stage 2 or 3
        increase_skip_fraction = [0.1, 0.3, 0.9, 100]
        ...
        train_sampler, train_loader = renew_vos_loader(5, transform=transform)
