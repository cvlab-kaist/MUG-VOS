...
from util.tensor_util import pad_divide_by, unpad, sam_pad, sam_unpad
...
class InferenceCore:
    ...
    def step(self, image, mask=None, valid_labels=None, end=False):
        ...
        self.curr_ti += 1
        image, self.pad = pad_divide_by(image, 16) # 1024 원래 16이었음
        pad_before_size = image.shape[-2:]
        image = sam_pad(image)
        ...
        key, shrinkage, selection, f16 = self.network.encode_key(image,
                                                    need_ek=(self.enable_long_term or need_segment), 
                                                    need_sk=is_mem_frame)
        single_scale_feature = f16

        # segment the current frame is needed
        if need_segment:
            memory_readout = self.memory.match_memory(key, selection).unsqueeze(0)
            hidden, _, pred_prob_with_bg = self.network.segment(single_scale_feature, memory_readout,
        ...
        if mask is not None:
            mask, _ = pad_divide_by(mask, 16) # 1024 원래 16이었음
            mask = sam_pad(mask)
        ...
        pred_prob_with_bg = sam_unpad(pred_prob_with_bg, pad_before_size)
