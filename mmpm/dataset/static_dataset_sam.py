...
class StaticTransformDataset(Dataset):
    ...
    def __init__(self, parameters, num_frames=3, max_num_obj=1, transform=None):
    ...
    def _get_sample(self, idx):
        ...
        # im = cv2.imread(self.im_list[idx])
    ...
    def __getitem__(self, idx):
        ...
        merged_images = None
        merged_masks = np.zeros((self.num_frames, 1024, 1024), dtype=np.int64)
        merged_masks_ori = np.zeros((self.num_frames, 384, 384), dtype=np.int64)

        for i, list_id in enumerate(indices):
            images, masks, masks_ori = self._get_sample(list_id)
            if merged_images is None:
                merged_images = images
            else:
                merged_images = merged_images * (1 - masks) + images * masks

            merged_masks[masks > 0.5] = (i + 1)
            merged_masks_ori[masks_ori > 0.5] = (i + 1)

        masks = merged_masks

        masks_ori = merged_masks_ori
        ...
        cls_gt_ori = np.zeros((self.num_frames, masks_ori.shape[-2], masks_ori.shape[-1]), dtype=np.int64)
